...
#define LOG_TAG "renderscript.toolkit.Convolve5x5"
    ...
    extern "C" void
    rsdIntrinsicConvolve5x5_K(void *dst, const void *y0, const void *y1, const void *y2,
    ...
    class Convolve5x5Task : public Task {
        ...
        void
        kernelU4(uchar *out, uint32_t xstart, uint32_t xend, const uchar *py0, const uchar *py1,
                 const uchar *py2, const uchar *py3, const uchar *py4);

        void
        convolveU4(const uchar *pin, uchar *pout, size_t vectorSize, size_t sizeX, size_t sizeY,
    ...
    static void ConvolveOneU(uint32_t x, InputOutputType *out, const InputOutputType *py0,
                             ...
                             const InputOutputType *py3, const InputOutputType *py4,
                             const float *coeff,
                             int32_t width) {
 ...
 */
    ...
    template<typename InputOutputType, typename ComputationType>
    static void
    convolveU(const uchar *pin, uchar *pout, size_t vectorSize, size_t sizeX, size_t sizeY,
    ...
    void
    Convolve5x5Task::processData(int /* threadIndex */, size_t startX, size_t startY, size_t endX,
                                 size_t endY) {
        // ALOGI("Thread %d start tile from (%zd, %zd) to (%zd, %zd)", threadIndex, startX, startY,
        // endX, endY);
        switch (mVectorSize) {
            case 1:
                convolveU<uchar, float>((const uchar *) mIn, (uchar *) mOut, mVectorSize, mSizeX,
                                        mSizeY,
                                        startX, startY, endX, endY, mFp);
                break;
            case 2:
                convolveU<uchar2, float2>((const uchar *) mIn, (uchar *) mOut, mVectorSize, mSizeX,
                                          mSizeY,
                                          startX, startY, endX, endY, mFp);
                break;
            case 3:
            case 4:
                convolveU4((const uchar *) mIn, (uchar *) mOut, mVectorSize, mSizeX, mSizeY, startX,
                           startY,
                           endX, endY);
                break;
        }
    ...
    void
    RenderScriptToolkit::convolve5x5(const void *in, void *out, size_t vectorSize, size_t sizeX,
