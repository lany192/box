...
public class ImageCropView extends AppCompatImageView {
    ...
    private static final String DEFAULT_BACKGROUND_COLOR_ID = "#B8000000";
    ...
    private final float[] suppMatrixValues = new float[9];
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        mTransparentLayerPaint.setAntiAlias(true);

        setScaleType(ScaleType.MATRIX);

        mGridInnerLinePaint = new Paint();
        float gridInnerStrokeWidth = a.getDimension(R.styleable.ImageCropView_icv_grid_inner_stroke, 1);
        mGridInnerLinePaint.setStrokeWidth(gridInnerStrokeWidth);
        int gridInnerColor = a.getColor(R.styleable.ImageCropView_icv_grid_inner_color, Color.WHITE);
        mGridInnerLinePaint.setColor(gridInnerColor);

        mGridOuterLinePaint = new Paint();
        float gridOuterStrokeWidth = a.getDimension(R.styleable.ImageCropView_icv_grid_Outer_stroke, 1);
        ...
        mGridOuterLinePaint.setAntiAlias(true);
    ...
    private void drawTransparentLayer(Canvas canvas) {
        ...
        if (circle) {
            ...
            float mHorizontalPadding = gridLeftRightMargin;

            // 计算矩形区域的宽度
            float mWidth = getWidth() - 2 * mHorizontalPadding;
            // 计算距离屏幕垂直边界 的边距
            float mVerticalPadding = (getHeight() - mWidth) / 2 - gridTopBottomMargin;

            // 绘制左边1
            canvas.drawRect(0, 0, mHorizontalPadding, getHeight(), mTransparentLayerPaint);
            // 绘制右边2
            canvas.drawRect(getWidth() - mHorizontalPadding, 0, getWidth(), getHeight(), mTransparentLayerPaint);
            // 绘制上边3
            canvas.drawRect(mHorizontalPadding, 0, getWidth() - mHorizontalPadding, mVerticalPadding, mTransparentLayerPaint);
            // 绘制下边4
            canvas.drawRect(mHorizontalPadding, getHeight() - mVerticalPadding, getWidth() - mHorizontalPadding, getHeight(), mTransparentLayerPaint);

            RectF rect = new RectF(mHorizontalPadding, mVerticalPadding, getWidth() - mHorizontalPadding, getHeight() - mVerticalPadding);
            Path mPath = new Path();
            ...
            canvas.drawPath(mPath, mTransparentLayerPaint);
    ...
    private void drawGrid(Canvas canvas) {
        ...
        if (gridOuterMode == GRID_ON) {
            if (circle) {
                float radius = (mCropRect.right - mCropRect.left) / 2;
                float cx = mCropRect.left + radius;
                float cy = mCropRect.top + radius;
                mGridOuterLinePaint.setStyle(Paint.Style.STROKE);
                //cx：圆心的x坐标,cy：圆心的y坐标,radius：圆的半径。paint：绘制时所使用的画笔。
                canvas.drawCircle(cx, cy, radius, mGridOuterLinePaint);
            } else {
