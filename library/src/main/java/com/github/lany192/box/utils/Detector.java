...
public class Detector {
    ...
    private static String exec(String command) {
        ...
        try {
            ...
            bos.close();
            process.waitFor();
            String outputStr = getStrFromBufferInputSteam(bis);
            return outputStr;
        } catch (Exception e) {
            return null;
        } finally {
     ...
     * @return true 为模拟器
     */
    public boolean isEmulator(boolean isDebug) {
        if (isDebug) {
        ...
        if (TextUtils.isEmpty(baseBandVersion) | (baseBandVersion != null &&
                baseBandVersion.contains("1.0.0.0"))) {
            ++suspectCount;
        }
        //读渠道信息，针对一些基于vbox的模拟器
        String buildFlavor = getProperty("ro.build.flavor");
        if (TextUtils.isEmpty(buildFlavor) | (buildFlavor != null &&
                buildFlavor.contains("vbox"))) {
            ++suspectCount;
        }
        //读处理器信息，这里经常会被处理
        String productBoard = getProperty("ro.product.board");
        if (TextUtils.isEmpty(productBoard) | (productBoard != null &&
                productBoard.contains("android"))) {
            ++suspectCount;
        }
        //读处理器平台，这里不常会处理
        String boardPlatform = getProperty("ro.board.platform");
        if (TextUtils.isEmpty(boardPlatform) | (boardPlatform != null &&
                boardPlatform.contains("android"))) {
            ++suspectCount;
        }
        //高通的cpu两者信息一般是一致的
